...
export default function PaymentMoney(props) {
  ...
  return (
    <div>
      ...
      <Card sx={{ mt: "3%", p: 1 }}>
        <MoneyBox>
          <span>{props.drinkItem.drinkName}</span>
          <span>+ {props.sizePrice}</span>
        </MoneyBox>
          ...
          if (option.type === "whip" && !props.drinkItem.whip && props.orderDetail[option.type]) {
            return (
              <MoneyBox key={option.type}>
                <span>{engOptionToKor[option.type]}</span>
                <span>+ {option.price * props.orderDetail[option.type]}</span>
              </MoneyBox>
            );
